        ...
        if block_bit_offset == 0 {
            if dist > 0 {
                if block_word_offset < dst_block.len() {
                    dst_block[block_word_offset..].copy_from_slice(&block[..block.len() - block_word_offset]);
                }
            } else {
                if block_word_offset < dst_block.len() {
                    dst_block[..block.len() - block_word_offset].copy_from_slice(&block[block_word_offset..]);
                }
            }
        } else {
            for i in 0..dst.len() {
        ...
        for row in y1..y2 {
            ...
            if sw == ew {
                if value {
                    ...
                    data[sw] &= !mask_sw;
                    if data[sw] == 0 { block[sw / 64] &= !(1 << (sw % 64)); }
                }
            } else {
                if value {
                    data[sw] |= mask_sw;
                    ...
                    if ew > sw + 1 {
                        ...
                        for w in sw+1..ew { block[w / 64] |= 1 << (w % 64); }
                    }
                    data[ew] |= mask_ew;
                    block[ew / 64] |= 1 << (ew % 64);
                } else {
                    ...
                    data[ew] &= !mask_ew;
                    if data[ew] == 0 { block[ew / 64] &= !(1 << (ew % 64)); }
                }
            }
        }
    }
    ...
    fn set_row(data: &mut [u64], block: &mut [u64], y: i32, min_x: i32, max_x: i32, value: bool) {
        ...
        if sw == ew {
            ...
            if value {
                ...
                data[sw] &= !mask;
                if data[sw] == 0 { block[sw / 64] &= !(1u64 << (sw % 64)); }
            }
            return;
        }
        ...
        if value {
            data[sw] |= s_mask; block[sw / 64] |= 1u64 << (sw % 64);
            if ew > sw + 1 {
                data[sw+1..ew].fill(!0u64);
                for w in sw+1..ew { block[w / 64] |= 1u64 << (w % 64); }
            }
            data[ew] |= e_mask; block[ew / 64] |= 1u64 << (ew % 64);
        } else {
            data[sw] &= !s_mask; if data[sw] == 0 { block[sw / 64] &= !(1u64 << (sw % 64)); }
            if ew > sw + 1 {
                data[sw+1..ew].fill(0);
                for w in sw+1..ew { block[w / 64] &= !(1u64 << (w % 64)); }
            }
            data[ew] &= !e_mask; if data[ew] == 0 { block[ew / 64] &= !(1u64 << (ew % 64)); }
        }
    }
...
mod tests {
    ...
    fn test_row_major_coord_boundaries() {
        assert_eq!(<RowMajorLayout as BitLayout<256, 256>>::coord_to_word_bit(0, 0), Some((0, 0)));
        assert_eq!(<RowMajorLayout as BitLayout<256, 256>>::coord_to_word_bit(255, 255), Some((1023, 63)));
        assert_eq!(<RowMajorLayout as BitLayout<256, 256>>::coord_to_word_bit(-1, 0), None);
        assert_eq!(<RowMajorLayout as BitLayout<256, 256>>::coord_to_word_bit(0, -1), None);
        assert_eq!(<RowMajorLayout as BitLayout<256, 256>>::coord_to_word_bit(256, 0), None);
        assert_eq!(<RowMajorLayout as BitLayout<256, 256>>::coord_to_word_bit(0, 256), None);
    }
}
